...
dotenv.config(); // let us use the environment variables from .env

// connect to database
const db = new pg.Pool({
  connectionString: process.env.DP_URL,
});
...
function handleGetRequest(filePath) {
  return async function (request, response) {
    try {
      const dbdata = await db.query(`SELECT * FROM ${filePath}data`);
      // const res = await db.query('SELECT msg_name, content FROM messages WHERE id = $1', [1]);
      response.sendFile(filePath); //local json response
      response.json(dbdata.rows); // db response
    } catch {
